...
class TestImport(unittest.TestCase):
    ...
    def test_decimal(test):
        ...
        for number, (parsed, data, _) in zip(numbers, outputs):
            with test.subTest():
                ...
                if "i" in number or "j" in number:
                    test.assertEqual(
                        data[0].captures[0].token, "storage.type.number.imaginary.matlab", MSG_NO_MATCH
                    )
        ...
        for number, (parsed, data, _) in zip(numbers, outputs):
            with test.subTest():
                ...
                if "s" in number or "u" in number:
                    test.assertEqual(
                        data[0].captures[0].token, "storage.type.number.hex.matlab", MSG_NOT_PARSED
                    )
    ...
    def test_binary(test):
        ...
        for number, (parsed, data, _) in zip(numbers, outputs):
            with test.subTest():
                ...
                if "s" in number or "u" in number:
                    test.assertEqual(
                        data[0].captures[0].token, "storage.type.number.binary.matlab", MSG_NOT_PARSED
                    )
